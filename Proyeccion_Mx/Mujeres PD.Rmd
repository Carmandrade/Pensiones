...
### Matriz acumuladas por edad mujeres
```{r}
...
mean_cot_f[72:116]<-180

cot_f <- array(0, dim=c(115,115))

for (edad in 1:115) {
  cot_f[1,edad]<-mean_cot_f[edad]
  for (i in 2:(115-i)) {
    cot_f[i,edad]<-cot_f[i-1,edad]+den_cot_f[edad]*12
    if(edad==40){cat(i," ",edad," ",cot_f[i,edad])}
  }
}
...
for (s in 1:100) {
  ...
  for (i in 2:(115-20)) { 
    ...
    if(    (cot_f[i,20] >= 180) 
        ...
        prob<-px[1, ynac, 20+i-2 +1, 7, ]
        
        if((j+i>=65)){
          #A1 tienen derecho a invalidez, vejez y pueden sucesion
            prob<-px[1, ynac, 20+i-2 +1, 1, ]
        }
        
        matriz_esto_1[i,]<- (rmultinom(1,matriz_esto_2[i-1,1], prob)) 
    
    }else{
        matriz_esto_1[i,]<- (rmultinom(1,matriz_esto_1[i-1,1], px[1, ynac, 20+i-2 +1, 6, ]))
    }
...
for (j in 21:114) {
    ...
    while (i+j<=115) {
      if(    (cot_f[i,j] >= 180) 
          ...
          || (Y[i+j] && (j+i>=48))  ){
        #A7 tienen derecho a invalidez y pueden morir
        prob<-px[1, ynac, j+i-2 +1, 7, ]
        
        if((j+i>=65)){
          #A1 tienen derecho a invalidez, vejez y pueden sucesion
            prob<-px[1, ynac, j+i-2 +1, 1, ]
        }
  ...
  simulaciones_f_9[,,s] <- matrizFinal_Estoca_9H

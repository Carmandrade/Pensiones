...
```{r}
poblacion_f<- read.csv("Poblacion_muj.csv")
poblacion_f<-poblacion_f[21:116,-c(1,2)]
poblacion_inicial_f<- cbind(poblacion_f[,1:2], poblacion_f[,4],poblacion_f[,3], poblacion_f[,5])

head(poblacion_inicial_f)
```
...
## 4.5 Matriz de densidades de cotizaciones anuales por edad
```{r}
# 'df' es el dataframe, con la primera columna como fecha de nacimiento
# y las siguientes 360 columnas como los datos de cotización mensuales.

df_f<-act_mujeres[,c(-2)]
sal_f<-sal_mujeres[,c(-2)]
...
sal_promedios_f <- matrix(NA, nrow = nrow(df_f), ncol = max_edad + 1)
...
sal_f_resultado <- as.data.frame(sal_promedios_f)
names(sal_f_resultado) <- paste("Edad", 0:max_edad, sep="_")
```

## 4.6 Vector densidad de cotizacion anual por edad 
```{r}
mean_values_f <- df_f_resultado %>%
  summarise(across(where(is.numeric), mean, na.rm = TRUE))

mean_muj<-mean_values_f
```

## 4.7 Vector salario anual promedio por edad 
```{r}
sal_mean_f <- sal_f_resultado %>%
  summarise(across(where(is.numeric), mean, na.rm = TRUE))

sal_mean_muj<-sal_mean_f
plot(as.numeric(sal_mean_muj))
```

## 4.8 Matriz cot acumuladas por edad mujeres
```{r}
matriz_acumuladas_f <- matrix(NA, nrow = nrow(df_f), ncol = max_edad + 1)

# Calcular las cotizaciones acumuladas por edad
for (i in 1:nrow(df_f)) {
  for (edad in 0:edades_f[i,length(edades_f[i,])]) {
    indices <- which(edades_f[i, ] <= edad)
    if (length(indices) > 0) {
      datos_acumulados_f <- as.numeric(df_f[i, (indices + 1)])  # +1 porque la primera columna es fecha de nacimiento 
      if (any(!is.na(datos_acumulados_f))) {
        matriz_acumuladas_f[i, edad + 1] <- sum(datos_acumulados_f, na.rm = TRUE)  # +1 porque la matriz inicia en columna 1
      }
    }
  }
}
...
df_f_resultado_acumuladas <- as.data.frame(matriz_acumuladas_f)
names(df_f_resultado_acumuladas) <- paste("Edad", 0:max_edad_f, sep = "_")

print(df_f_resultado_acumuladas)
```

## 4.9 Vector de cotizaciones acumuladas por edad 
```{r}
# Clean column names
df_f_resultado_acumuladas <- df_f_resultado_acumuladas %>%
  clean_names()

# Calculate the mean of numeric columns
mean_acumulados_f <- df_f_resultado_acumuladas %>%
  summarise(across(where(is.numeric), ~ mean(.x, na.rm = TRUE)))

mean_acumulados_f
plot(as.numeric(mean_acumulados_f))
```
...
## 4.10 Matriz de cotizaciones cot_f
```{r}
den_cot_f<-rep(1,116)
den_cot_f[1:length(mean_values_f)]<-unlist(mean_values_f)
den_cot_f[74:116]<-1

mean_cot_f<-rep(1,116)
mean_cot_f<-unlist(mean_acumulados_f)
mean_cot_f[74:116]<-180

cot_f <- array(0, dim=c(115,115))

for (x in 1:115) {
  cot_f[1,x] <- mean_cot_f[x]
  k<-2
  while((x+k)<=115) {
    cot_f[k,x] <- cot_f[k-1,x]+den_cot_f[x]*12
    k<-k+1
  }
}

#cot_f[anno_proyeccion, edad_inicial] por ejemplo cuantas cotizaciones tiene una hombre de 50 dentro de dos annos:
cot_f[,40]
```
...
```{r}
simulaciones_f_9 <- array(0,dim = c(115,5,100))   #(proyec, estados, simulaciones)
```

## 5.3 Proyeccion 1 Mujeres
```{r}
for (s in 1:100) {
  ## crea matriz vacia
  ...
  matriz_esto_1[1, ] <- as.numeric(poblacion_inicial_f[1, 1:5]) 
  ...
  for (i in 2:(115-20)) { 
    ...
    if(    (cot_f[i,20] >= 180) 
        ...
        prob<-px[2, ynac, 20+i-1, 7, ]
        
        if((20+i>=65)){
          #A1 tienen derecho a invalidez, vejez y pueden sucesion
            prob<-px[2, ynac, 20+i-1, 1, ]
        }
        
        matriz_esto_1[i,]<- (rmultinom(1,matriz_esto_1[i-1,1], prob)) 
    
    }else{
        matriz_esto_1[i,]<- (rmultinom(1,matriz_esto_1[i-1,1], px[2, ynac, 20+i-1, 6, ]))
    }
    
    matriz_esto_1[i,]<-(rmultinom(1,matriz_esto_1[i-1,2], px[2, ynac, 20+i-2+1, 2, ])) + matriz_esto_1[i,]
    matriz_esto_1[i,]<-(rmultinom(1,matriz_esto_1[i-1,3], px[2, ynac, 20+i-2+1, 3, ])) + matriz_esto_1[i,]
    matriz_esto_1[i,]<-(rmultinom(1,matriz_esto_1[i-1,4], px[2, ynac, 20+i-2+1, 4, ])) + matriz_esto_1[i,]
    matriz_esto_1[i,]<-(rmultinom(1,matriz_esto_1[i-1,5], px[2, ynac, 20+i-2+1, 5, ])) + matriz_esto_1[i,]
    
  } 
  
  matrizFinal_Estoca_9f <- matriz_esto_1 

    
  for (j in 21:114) {
      
      matriz_esto_2 <- matrix(0, ncol = 5, nrow = 115) 
      matriz_esto_2[1, ] <- as.numeric(poblacion_inicial_f[j-19, 1:5])
      ynac2 <- 2023-j-1906
      i=2
      while (i+j<=115) {
        if(    (cot_f[i,j] >= 180) 
            ...
            || (Y[i+j] && (j+i>=48))  ){
          #A7 tienen derecho a invalidez y pueden morir
          prob<-px[2, ynac, j+i-1, 7, ]
          
          if((j+i>=65)){
            #A1 tienen derecho a invalidez, vejez y pueden sucesion
              prob<-px[2, ynac, j+i-1, 1, ]
          }
          ...
          matriz_esto_2[i,]<- (rmultinom(1,matriz_esto_2[i-1,1], px[2, ynac2, j+i-1, 6, ]))
        }
        
        matriz_esto_2[i,]<-(rmultinom(1, matriz_esto_2[i-1,2], px[2, ynac2, j+i-1, 2, ])) + matriz_esto_2[i,] 
        matriz_esto_2[i,]<-(rmultinom(1, matriz_esto_2[i-1,3], px[2, ynac2, j+i-1, 3, ])) + matriz_esto_2[i,]
        matriz_esto_2[i,]<-(rmultinom(1, matriz_esto_2[i-1,4], px[2, ynac2, j+i-1, 4, ])) + matriz_esto_2[i,]
        matriz_esto_2[i,]<-(rmultinom(1, matriz_esto_2[i-1,5], px[2, ynac2, j+i-1, 5, ])) + matriz_esto_2[i,]
      ...
      # suma las matrices
      matrizFinal_Estoca_9f <-  matrizFinal_Estoca_9f + matriz_esto_2   
    }
  
  # Rellena cada tercera dimensión con una simulación  
  simulaciones_f_9[,,s] <- matrizFinal_Estoca_9f
...
matriz_resumenEsperanza_f_9 <- array(0,dim = c(115,5)) 

for (i in 1:115) {
  for (j in 1:5) {
    matriz_resumenEsperanza_f_9[i,j] <- round(mean(simulaciones_f_9[i,j,]))
  }
}
```
## 5.5 Grafico de las simulacioes
```{r}
df<-as.data.frame(unlist(matriz_resumenEsperanza_f_9))
...
```{r}
df<-as.data.frame(unlist(matriz_resumenEsperanza_f_9[,-5]))
...
Hace Sim=1000 simulaciones para cada edad, luego suma todas las proyecciones para todas las edades y obtiene la "v2_matriz_resumenEsperanza_F_9" que es analoga a "matriz_resumenEsperanza_H_9"
```{r}
v2_matriz_resumenEsperanza_F_9<-array(0,dim = c(115,5))
f_esperanza_matrizEdad_x<-array(0,dim = c(115,5,115))
sim<-1000
```
...
for (edad_x in 20:113) {
  ...
  for (s in 1:sim){ 
    ...
    matriz_esto_1[1, ] <- as.numeric(poblacion_inicial_f[edad_x-19, 1:5]) 
    ynac <- 2023-edad_x-1906
    
    for (i in 2:(115-edad_x)) { 
      
      if(    (cot_f[i,edad_x] >= 180) 
            || (X[i+edad_x] && (edad_x+i<48))
            || (Y[i+edad_x] && (edad_x+i>=48))  ){
          #A7 tienen derecho a invalidez y pueden morir
          prob<-px[2, ynac, edad_x+i-1, 7, ]
          
          if((edad_x+i>=65)){
            #A1 tienen derecho a invalidez, vejez y pueden sucesion
              prob<-px[2, ynac, edad_x+i-1, 1, ]
          }
          ...
          if (matriz_esto_1[i-1, 1] > 0) {
              matriz_esto_1[i,]<- (rmultinom(1,matriz_esto_1[i-1,1], px[2, ynac, edad_x+i-1, 6, ]))
          } else {
              # Manejo del caso en que no hay población para simular
              matriz_esto_1[i,] <- rep(0, 5)
          }
      ...
      matriz_esto_1[i,]<-(rmultinom(1,matriz_esto_1[i-1,2], px[2, ynac, edad_x+i-2+1, 2, ])) + matriz_esto_1[i,]
        } 
      if (matriz_esto_1[i-1, 3] > 0) {
      matriz_esto_1[i,]<-(rmultinom(1,matriz_esto_1[i-1,3], px[2, ynac, edad_x+i-2+1, 3, ])) + matriz_esto_1[i,]
        }
      if (matriz_esto_1[i-1, 4] > 0) {
      matriz_esto_1[i,]<-(rmultinom(1,matriz_esto_1[i-1,4], px[2, ynac, edad_x+i-2+1, 4, ])) + matriz_esto_1[i,]
        } 
      if (matriz_esto_1[i-1, 5] > 0) {
      matriz_esto_1[i,]<-(rmultinom(1,matriz_esto_1[i-1,5], px[2, ynac, edad_x+i-2+1, 5, ])) + matriz_esto_1[i,]
        }  
    ...
    f_esperanza_matrizEdad_x[,,edad_x-19]<-esperanzaEdad_x
    #suma para todas las edades
    v2_matriz_resumenEsperanza_F_9<-v2_matriz_resumenEsperanza_F_9+esperanzaEdad_x
    

}
#f_esperanza_matrizEdad_x[,,3]
```

## 6.3 Grafico Proyeccion 2
```{r}
df<-as.data.frame(unlist(v2_matriz_resumenEsperanza_F_9))
colnames(df) <- c("activo", "invalidez",  "vejez","sucesion", "muerte")
df$Year <- 1:nrow(df)
...
```{r}
df<-as.data.frame(unlist(v2_matriz_resumenEsperanza_F_9[,-5]))
...
```
## 7.3 Grafico Proyeccion Hombres y mujeres v2
```{r}
pd<-v2_matriz_resumenEsperanza_F_9+v2_matriz_resumenEsperanza_H_9
df<-as.data.frame(unlist(pd[,-5]))
colnames(df) <- c("activo", "invalidez",  "vejez","sucesion", "muerte")
df$Year <- 1:nrow(df)
...
```{r}
pd<-v2_matriz_resumenEsperanza_F_9+v2_matriz_resumenEsperanza_H_9
df<-as.data.frame(unlist(pd[1:100,-5]))

write.csv(df, "Proyeccion_Total.csv")
df$Año <- 2024:(2024 + nrow(df) - 1)
